...
**Filter by minimum CVE severity:** Read the environment variable `APPMOD_MINIMUM_CVE_SEVERITY` (defaults to `high` if not set). Only include CVE findings whose severity meets or exceeds this threshold. The severity order from lowest to highest is: `low` < `medium` < `high` < `critical`. For example, if `APPMOD_MINIMUM_CVE_SEVERITY=high`, only include findings with severity `high` or `critical`. Exclude findings with unknown or missing severity.
